...
const albums = ({ params }: { params: { albumid: string } }) => {
  ...
  const fetchAlbumDetails = async (albumId: string) => {
    try {
      ...
      console.log("ftech api response");
      console.log(response);
      console.log(response.success);
      console.log(response.data);
      //   `${process.env.NEXT_PUBLIC_AWS_S3_FOLDER_PATH}/albums/${albumId}/${albumDetails.thumbnail}`
      console.log(
    ...
    <div>
      <div className={Style.albumContainer}>
        ...
        <div className={`p-3 border rounded ${Style.albumDetails}`}>
          {albumDetails && (
            <div style={{ width: "100%" }}>
              <AlbumStatus status={albumDetails.status}  comment={albumDetails.comment ?? ''} />
            </div>
          )}
          ...
          <ul className={Style.albumInfoList}>
            ...
            <li className={`mb-2 ${Style.albumInfoItem}`}>
              <span className="text-sm font-medium text-gray-900 truncate dark:text-white">
                Release Date: </span> {albumDetails?.releasedate
          ...
          <div className="flex">
            ...
            {
              albumId && (
                <AlbumStatusUpdate albumid={albumId} onUpdate={() => fetchAlbumDetails(albumId)} />
              )
            }
